...
const Card = ({ recentJob, employerData, duration }: any) => {
  ...
  return (
    <div ref={ref}>
      <motion.div
        animate={animation1}
        className="flex w-[18rem] xl:w-[20rem] 2xl:w-[22rem] border border-slate-300 flex-col gap-2 p-5 shadow-lg shadow-slate-700 bg-white rounded-2xl h-full"
      >
        <Link
          href={`/listofjobs/${recentJob._id}/viewjob`}
        ...
        >
          ...
          <div className="w-full text-[#0099ff] p-bold-16"></div>
        </Link>
      </motion.div>
    </div>
  );
